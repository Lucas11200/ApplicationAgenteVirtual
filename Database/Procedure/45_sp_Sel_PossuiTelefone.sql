IF EXISTS (SELECT * FROM sys.objects WHERE type = 'P' AND name = 'sp_Sel_PossuiTelefone')
	DROP PROCEDURE sp_Sel_PossuiTelefone
	GO

CREATE PROCEDURE sp_Sel_PossuiTelefone(
    @Operador	VARCHAR(5),
	@Valor	VARCHAR(200)
)
AS   
	...
	IF(LTRIM(RTRIM(UPPER(@Valor))) = 'SIM' )
		BEGIN
			...
			--Alimenta a tabela #TempTelA com todos clientes que estão Ativos e Possuem Telefone	
	...
	ELSE IF(LTRIM(RTRIM(UPPER(@Valor))) = 'NÃO' )
		BEGIN	
				...
				--Remove o usuários que possuem Telefone da #TempSemTel
